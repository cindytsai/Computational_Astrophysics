...
u0  = 0.1   # background density
amp = 0.5   # Gaussian amplitude
sig = 0.1   # Gaussian sigma
...
def ref_func( x, t ):
   return u0 + amp*np.exp( -((x-0.5*L)/sig)**2.0 )
...
ax        = plt.axes( xlim=(0.0,L), ylim=(0.0,u0+amp*1.5) )
line_num, = ax.plot( [], [], 'r', ls='-',  label='Numerical' )
line_ref, = ax.plot( [], [], 'b', ls='--', label='Initial' )
ax.set_xlabel( 'x' )
ax.set_ylabel( 'u' )
