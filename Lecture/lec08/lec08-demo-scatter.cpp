...
// prepare the send buffer
   ...
   int *SendBuf = new int [SendCount*NRank]; // only relevant for the root rank
   int *RecvBuf = new int [RecvCount];

   if ( MyRank == RootRank ) {
      for (int t=0; t<SendCount*NRank; t++)  SendBuf[t] = t;
   }
...
// print the send and recv buffers
   if ( MyRank == RootRank ) {
      ...
      for (int t=0; t<SendCount*NRank; t++)  printf( " %2d", SendBuf[t] );
      printf( "\n" );
   }

   printf( "Recv buffer on rank %d/%d:", MyRank, NRank );
   for (int t=0; t<RecvCount; t++)   printf( " %2d", RecvBuf[t] );
   printf( "\n" );
