...
#--------------------------------------------------------------------
# Solve the diffusion equation with Crank Nicolson scheme
#--------------------------------------------------------------------
...
# set the coefficient matrices A with A*u(t+dt)=B*u(t)
r = D*dt/dx**2
A = np.diagflat( np.ones(N-3)*(-r/2.0),       -1 ) + \
    np.diagflat( np.ones(N-2)*(1.0+r),         0 ) + \
    np.diagflat( np.ones(N-3)*(-r/2.0),       +1 );
B = np.diagflat( np.ones(N-3)*(r/2.0),        -1 ) + \
    np.diagflat( np.ones(N-2)*(1.0-r),         0 ) + \
    np.diagflat( np.ones(N-3)*(r/2.0),        +1 );
...
#     (3) apply the Dirichlet boundary condition: u[0]=u[N-1]=u0
      u_bk[ 0] += r*u0
      u_bk[-1] += r*u0
